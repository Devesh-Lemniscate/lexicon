...
export default function FolderView() {
  ...
  const handleItemClick = async (item: FileEntry) => {
    ...
    } else if (/\.(png|jpg|jpeg|gif|webp|svg|bmp|ico)$/i.test(item.path)) {
      navigate(`/image/${sourceId}/${item.path}`);
    } else if (/\.pdf$/i.test(item.path)) {
      await readingProgressRepository.setLastOpened(sourceId!, item.path);
      navigate(`/pdf/${sourceId}/${item.path}`);
    } else if (/\.(txt|json|yaml|yml|xml|html|css|csv)$/i.test(item.path)) {
      await readingProgressRepository.setLastOpened(sourceId!, item.path);
      navigate(`/text/${sourceId}/${item.path}`);
    }
  };
  ...
  const getFileIcon = (item: FileEntry) => {
    ...
    if (/\.(png|jpg|jpeg|gif|webp|svg|bmp|ico)$/i.test(item.path)) {
      return (
    ...
    if (/\.(json|yaml|yml|xml)$/i.test(item.path)) {
      return (
